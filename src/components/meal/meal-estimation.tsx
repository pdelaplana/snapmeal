...
export default function MealEstimation({
  ...
  const showCalories =
    ...
    (estimationType === "calories_macros" ||
      estimationType === "calories_only");
  const showMacros =
        ...
        <CardContent>
          <Alert variant="destructive">
            ...
            <AlertDescription>
              ...
              {estimationType.replace("_", " ")} estimates or recognized items.
              You might want to try a clearer photo, add a description, or
              re-estimate.
            </AlertDescription>
          </Alert>
        </CardContent>
  ...
  if (macroBreakdown) {
    const totalMacros =
      (macroBreakdown.protein || 0) +
      (macroBreakdown.carbs || 0) +
      (macroBreakdown.fat || 0);
    proteinPercentage =
      totalMacros > 0 ? ((macroBreakdown.protein || 0) / totalMacros) * 100 : 0;
    carbsPercentage =
      totalMacros > 0 ? ((macroBreakdown.carbs || 0) / totalMacros) * 100 : 0;
    fatPercentage =
      totalMacros > 0 ? ((macroBreakdown.fat || 0) / totalMacros) * 100 : 0;
  }
      ...
      <CardContent className="space-y-6">
        ...
        {(showCalories || showMacros) && (
          <p className="text-xs text-muted-foreground">
            *These are AI estimates. Actual values may vary. You can manually
            adjust them below.
          </p>
        )}
      </CardContent>
