...
import { format, parseISO, startOfWeek, endOfWeek, getDaysInMonth, compareDesc } from 'date-fns';
import { Zap, Scale, Utensils, CalendarDays, CalendarRange, CalendarIcon } from 'lucide-react';
...
export default function SummaryPage() {
  ...
  const weeklySummaries: WeeklySummaryItem[] = useMemo(() => {
    ...
    return Object.values(summaries)
      .map(s => ({ ...s, avgDailyCalories: s.calories > 0 ? s.calories / 7 : 0 }))
  ...
  const renderSummaryCard = (item: DailySummaryItem | WeeklySummaryItem | MonthlySummaryItem) => {
    const title = (item as DailySummaryItem).formattedDate || (item as WeeklySummaryItem).weekLabel || (item as MonthlySummaryItem).monthLabel;
    const avgCalories = (item as WeeklySummaryItem | MonthlySummaryItem).avgDailyCalories;

    return (
      <Card key={(item as DailySummaryItem).date || (item as WeeklySummaryItem).weekKey || (item as MonthlySummaryItem).monthKey} className="shadow-md">
        <CardHeader>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="flex items-center justify-between">
            <span className="flex items-center"><Zap className="mr-2 h-4 w-4 text-primary" />Total Calories:</span>
            <span className="font-semibold text-primary">{item.calories.toFixed(0)} kcal</span>
          </div>
          {avgCalories !== undefined && avgCalories > 0 && (
            <div className="flex items-center justify-between">
              <span className="flex items-center"><Zap className="mr-2 h-4 w-4 text-primary/70" />Avg Daily Calories:</span>
              <span className="font-semibold">{avgCalories.toFixed(0)} kcal</span>
            </div>
          )}
          <div>
            <p className="flex items-center font-medium mb-1"><Scale className="mr-2 h-4 w-4 text-primary" />Macros:</p>
            <div className="pl-6 space-y-1 text-muted-foreground">
              <div className="flex justify-between"><span>Protein:</span> <span className="font-medium text-foreground">{item.protein.toFixed(1)}g</span></div>
              <div className="flex justify-between"><span>Carbs:</span> <span className="font-medium text-foreground">{item.carbs.toFixed(1)}g</span></div>
              <div className="flex justify-between"><span>Fat:</span> <span className="font-medium text-foreground">{item.fat.toFixed(1)}g</span></div>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  if (mealsLoading) {
    return <AppLayout><div className="flex min-h-[calc(100vh-150px)] items-center justify-center"><LoadingSpinner className="h-10 w-10 text-primary" /></div></AppLayout>;
  }
      ...
      <div className="container mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-6">
            <TabsTrigger value="daily"><CalendarDays className="mr-2 h-4 w-4" />Daily</TabsTrigger>
            <TabsTrigger value="weekly"><CalendarRange className="mr-2 h-4 w-4" />Weekly</TabsTrigger>
            <TabsTrigger value="monthly"><CalendarIcon className="mr-2 h-4 w-4" />Monthly</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="daily">
            {dailySummaries.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-8">No daily summary data available.</p>
            )}
          </TabsContent>
          <TabsContent value="weekly">
            {weeklySummaries.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-8">No weekly summary data available.</p>
            )}
          </TabsContent>
          <TabsContent value="monthly">
            {monthlySummaries.length > 0 ? (
              ...
              <p className="text-center text-muted-foreground py-8">No monthly summary data available.</p>
            )}
          </TabsContent>
        </Tabs>
      </div>
